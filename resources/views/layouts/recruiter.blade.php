...
<head>
    ...
    <title>{{ $title ?? 'Recruiter Dashboard' }} - Job Board</title>
            ...
            <main class="flex-1 md:ml-0">
                ...
                <header class="border-b border-gray-200 bg-white shadow-sm sticky top-0 z-30">
                    <div class="flex items-center justify-between px-6 md:px-8 py-4 md:py-6">
                        <div>
                            <h1 class="text-2xl md:text-3xl font-bold text-slate-900">{{ $title ?? 'Recruiter Dashboard' }}</h1>
                            <p class="text-sm text-slate-500 mt-1">{{ config('app.company_name') ?? 'Perusahaan Anda' }}</p>
                        </div>
                        <div class="flex items-center gap-4">
                            <div class="text-right hidden md:block">
                                <p class="text-sm font-medium text-slate-900">{{ auth('recruiter')->user()->company->name ?? 'Perusahaan' }}</p>
                                <p class="text-xs text-slate-500">Recruiter</p>
                            </div>
                        </div>
                    </div>
                </header>
