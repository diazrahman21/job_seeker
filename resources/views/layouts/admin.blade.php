        ...
        <main class="ml-64 flex-1 flex flex-col">
            ...
            <header class="border-b border-gray-200 bg-white shadow-sm sticky top-0 z-30">
                <div class="flex items-center justify-between px-8 py-6">
                    <div>
                        <h1 class="text-3xl font-bold text-slate-900">{{ $title ?? 'Admin Panel' }}</h1>
                        <p class="text-sm text-slate-500 mt-1">{{ request()->route()->getName() }}</p>
                    </div>
                    <div class="flex items-center gap-6">
                        <div class="text-right">
                            <p class="text-sm font-medium text-slate-900">{{ auth('admin')->user()->name ?? 'Admin' }}</p>
