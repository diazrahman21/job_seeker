        ...
        @forelse ($jobs as $job)
            <article class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <div class="mt-3 flex items-center justify-between text-sm">
                    <span class="rounded bg-slate-100 px-2 py-1">{{ $job->employment_type }}</span>
                    <a href="{{ route('job-seeker.jobs.detail', $job) }}" class="font-semibold text-indigo-600">Lihat Detail</a>
                </div>
            </article>
        @empty
